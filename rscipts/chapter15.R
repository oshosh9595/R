...
# 매트릭스 : 열벡터를 모으는거
# 테이블 : 헤더있는거
# 데이터 프레임 : 안쪽을 볼수잇는 메서드를 더주는거

#선형회귀 잔차를 최소한으로 한줄만드는거
## 단계 3: 단순 선형회귀 "모델" 생성 - 모델이란 직선이 모델이되는부분
# 모델은 수식 기울기 절편이있을거같다는 생각을해야된다 선형단순 이라서
result.lm <- lm(formula = y ~ x, data = df)

## 단계 4: 회귀분석의 절편과 기울기 - 확인하기
result.lm

## 단계 5: 모델의 잔차 보기
residuals(result.lm)[1:2]

# 오차를 어떤형태를바라볼것인지 선형회귀 지표(MAE, MSE, RMSE, RSQUARED) - 한모델에 하나로만 쭉 써야한다
# 잔차는 회귀에서만쓴다 분류에서는 x
...
#pkg <- c("party", "datasets") 라이브러리 한번에 넣어서
#install.packages(pkg)  한번에 인스톨하기
